/* DYNK Scroll-Driven Animations */
(function() {
  var isMobile = window.innerWidth <= 768;

  // ── MOBILE: IntersectionObserver — one-shot fade/rise, no continuous JS ──
  if (isMobile) {
    ...
    if (hInner) { hInner.style.opacity = '1'; hInner.style.transform = 'none'; }
    if (hFeat)  { hFeat.style.opacity  = '1'; hFeat.style.transform  = 'none'; }

    // Board cells: reveal as a single group when board scrolls into view
  ...
  // ── DESKTOP: scroll-driven lerp animations ──
  ...
  function update() {
    ...
    if (scrollY > 10) {
      ...
      if (hFeat) hFeat.style.opacity = '' + Math.max(0, 1 - hp * 2);
    }
    ...
    for (var i = 0; i < animEls.length; i++) {
      ...
      var t = ease(p);
      if (type === 'fade-up')     el.style.transform = 'translateY(' + lerp(40, 0, t) + 'px)';
      else if (type === 'slide-left')  el.style.transform = 'translateX(' + lerp(60, 0, t) + 'px)';
      else if (type === 'slide-right') el.style.transform = 'translateX(' + lerp(-60, 0, t) + 'px)';
      el.style.opacity = '' + t;
    }
